...
<body>
  ...
  <div v-if="!loading && !showNoBookMessage">
    <!-- Right now it's not hooked up to localStorage/sessionStorage/ or actually posting to a SQL instance so you can only save one book -->
      <div v-for="book in savedBooks" :key="book.title" class="white-text">
        <!-- Typically wouldn't be slicing and dicing with <br>, would use padding/margin, more specific css -->
        Your book including your favorite dog breed {{ book.breed }}'s is saved! <br> <br>
          Breed: <br> {{ book.breed }} <br> <br>
          Title: <br> {{ book.title }} <br> <br>
          Author:<br> {{ book.author }} <br> <br>
      </div>
      <br> <br>
      <img :src="this.newBook.dogPicUrl"/>
  </div>
  <div v-if="showNoBookMessage" class="white-text-large">
    You don't have any saved books yet, Go pick your favorite dog and save it! <br><br>
    Here's a random dog picture for your troubles <br><br>
    <div v-if="!randomDogPicUrl">
      <div class="loader"></div>
    </div>
    <img :src="this.randomDogPicUrl"/>
  </div>
</body>
</html>
</template>
...
data(){ 
  return {
    ...
    loading: true,
    showNoBookMessage: false,
    randomDogPicUrl: ''
  }
},
mounted(){
  //Normally I would set up a Parent / Child relationship and pass props, or use the Vue $event bus
  this.loading = true
  this.title = this.$router.currentRoute._value.query.title
  this.author = this.$router.currentRoute._value.query.author
  this.dogPicUrl = this.$router.currentRoute._value.query.dogPicUrl
  this.breed = this.$router.currentRoute._value.query.breed
  if(this.title || this.author || this.dogPicUrl || this.breed) {
    this.newBook.author = this.author
    this.newBook.title = this.title
    this.newBook.dogPicUrl = this.dogPicUrl
    this.newBook.breed = this.breed
    this.savedBooks.push(this.newBook)
    this.loading = false
  }
  if(this.loading) {
    this.getDogPicture()
...
methods: {
   async getDogPicture() {
     const response = await getRandomDogPicture()
     this.randomDogPicUrl = response.message
     if(this.title || this.author) {
      this.showNoBookMessage = false
      this.loading = false
     }
     if(!this.title && !this.author)  {
      this.showNoBookMessage = true
      this.loading = false
     }
