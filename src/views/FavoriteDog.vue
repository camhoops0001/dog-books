<!-- I would normally break a some of this out into seperate components -->
<template>
<html>
<body>
  <div>
    ...
    <div id='stars2'></div>
    <div id='stars3'></div>
    <div v-if="dogPictureUrl == ''">
      <div class="white-text">We will show you books with your favorite dog breeds! <br><br> Pick your favorite dog breed from the list below, and we will match you with books that have those dogs in them  </div>

      <div class="white-text" v-if="dogBreeds">
        <ul>
          <ul class ="breeds" v-for="breed in Object.keys(dogBreeds.message)" :key="breed" @click="findDogsandBooks(breed)">
            {{ breed }}
          </ul>
        </ul>
      </div>
    </div>
    <!-- Typically wouldn't be slicing and dicing with <br>, would use padding/margin, more specific css -->
    <div class="white-text" v-if="dogPictureUrl !== '' && !resultsCompleted">
    ...
    <div v-if="resultsCompleted">
      <div class="white-text">
        Your book tailored to your favorite dog breed {{ selectedBreed }}'s is ready! <br> <br>
          Title: <br> {{ displayTitleOrFullTitle() }} <br> <br>
          Author:<br> {{ author }} <br> <br>
      </div>
      <button class="center" @click="saveBook"> Save This Book! </button>
      <br> <br>
  ...
  export default defineComponent({
    ...
    data() {
      ...
      return {
        ...
        trimmedParam: "",
        filteredStringArray: stringArray,
        authorKeyAndTitle: authorKeyAndTitle,
        author: "",
        saveThisBook: false,
        randomBook: 0
    ...
    mounted() {
      this.clearFields()
      this.fetchDogBreedData()
    },
    methods: {
      clearFields() {
        this.authorKeyAndTitle = undefined
        this.author = ""
      },
      ...
      async findDogsandBooks(breed: string) {
        const response = await getDogPictureByBreed(breed);
       ...
       this.filteredStringArray = this.stringArray.filter(entry => entry.startsWith("s/")); //some dogs return some non ID's
        ...
        this.authorKeyAndTitle = parseOpenLibraryBookResponse(authorKeyAndTitleResponse) //They won't just give you the author name, they give you a key so you have to make another call

        const openLibraryAuthor = await getAuthorbyAuthorID(this.authorKeyAndTitle.authorKey)
        this.author = openLibraryAuthor.name

        this.artificialDelay() //implemented the artificial delay
        this.resultsCompleted = true;
        //Add another picture after the book is retrieved
        const pictureUrl2 = await getDogPictureByBreed(breed);
        this.dogPictureUrl = pictureUrl2.message


        //Follow this up with the ability to "POST", save your collection
      },
      async saveBook() { //This will be my artificial "POST". Will just save to local data since this is not hooked up 
      //Typically would pass this data through props or Vue $event bus.
      ...
      // eslint-disable-next-line
        await this.delayForTwoSeconds()
      },
      displayTitleOrFullTitle() { //for the use case where the result doesn't have a full_title
        if(this.authorKeyAndTitle?.full_title != null)  {
          return this.authorKeyAndTitle.full_title
        }
        return this.authorKeyAndTitle?.title
      }
    },
  });
